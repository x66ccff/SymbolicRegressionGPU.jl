mutable struct Scalar{T}
    ...
    function Scalar{T}(p::Ptr) where {T}
        ...
        ret = new(T, p)
        finalizer(ret) do s
            ccall((:scalar_destroy, :libtorch_capi), Cvoid, (Ptr{Cvoid},), s.pointer)
        end
        return ret
    end
end
...
function Scalar{T}(s::U) where {T<:TorchNumber,U<:TorchNumber}
    ...
    data = T[convert(T, s)]
    ptr = ccall(
        (:scalar_from_data, :libtorch_capi),
        Ptr{Cvoid},
        (Ptr{Cvoid}, Cchar),
        data,
        TYPE_MAP[T],
...
function Base.show(io::IO, s::Scalar{T}) where {T}
    return write(io, "PyTorch.Scalar{$T} = $(value(s))\n")
end

function Base.display(s::Scalar)
    return show(stdout, s)
end
