...
function Base.div(a::Tensor{T,N}, b::Tensor{T,N}, r::RoundingMode=RoundToZero) where {T,N}
    return div(a, b)
end
...
function Base.ones(::Type{Tensor{T}}, I::Vararg{Int}; dev::Device=CPU()) where {T}
    dims = Int64[I...]
    return THC.ones(dims, eltype_id(T), convert(Int, dev))
end

function Base.zeros(::Type{Tensor{T}}, I::Vararg{Int}; dev::Device=CPU()) where {T}
    dims = Int64[I...]
    return THC.zeros(dims, eltype_id(T), convert(Int, dev))
end

function Base.rand(::Type{Tensor{T}}, I::Vararg{Int}; dev::Device=CPU()) where {T}
    dims = Int64[I...]
    return THC.rand(dims, eltype_id(T), convert(Int, dev))
end

function THC.eye(::Type{T}, n::Int64; dev::Device=CPU()) where {T}
    return THC.eye(n, eltype_id(T), convert(Int, dev))
end
THC.eye(::Type{Tensor{T}}, n::Int64; dev::Device=CPU()) where {T} = THC.eye(T, n; dev=dev)
