...
function opt_index_select(self::Tensor{T,N}, dim::Int64, index::Int64) where {T,N}
    dim = dim - 1
    ptr = ccall(
        (:tensor_method_index_select_int64, :libtorch_capi),
        Ptr{Cvoid},
        (Ptr{Cvoid}, Clonglong, Clonglong),
        self.pointer,
        dim,
        index,
    )
...
function opt_index_select(self::Tensor{T,N}, dim::Int64, index::Tensor) where {T,N}
    dim = dim - 1
    __cret = ccall(
        (:atg_index_select, :libtorch_capi),
        Ptr{Cvoid},
