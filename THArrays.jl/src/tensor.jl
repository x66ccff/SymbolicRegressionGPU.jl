...
mutable struct Tensor{T,N} <: AbstractArray{T,N}
    ...
    function Tensor{T,N}(p::Ptr, data) where {T,N}
        ...
        ret = new(T, N, p, data)
        finalizer(ret) do t
            ccall((:tensor_destroy, :libtorch_capi), Cvoid, (Ptr{Cvoid},), t.pointer)
        end
        return ret
    end
end
...
function Tensor{T}(array::Array{U,N}; detach=false, requires_grad=false) where {T,U,N}
    if !haskey(TYPE_MAP, T)
        error("Type $T is not supported.")
    end
    ...
    ptr = ccall(
        (:tensor_from_data, :libtorch_capi),
        Ptr{Cvoid},
        (Ptr{Cvoid}, Csize_t, Cchar, Ptr{Clonglong}, Ptr{Clonglong}, Csize_t, Cint, Cint),
        array,
        sizeof(array),
        TYPE_MAP[T],
        dims,
    ...
    return Tensor{T,N}(ptr, detach ? nothing : array)
end

function Tensor(array::Array{T,N}; detach=false, requires_grad=false) where {T,N}
    return Tensor{T}(array; detach=detach, requires_grad=requires_grad)
end
...
function Tensor(s::Int64; requires_grad=false)
    grad = requires_grad ? 1 : 0
    ptr = ccall(
        (:tensor_int64_0dim, :libtorch_capi), Ptr{Cvoid}, (Clonglong, Cint), s, grad
    )
    return Tensor{Int64,0}(ptr, nothing)
end
...
function Tensor(s::T; requires_grad=false) where {T<:TorchNumber}
    ...
    return Tensor{T,0}(ptr, nothing)
end

function Tensor(a0::Array{T,0}; requires_grad=false) where {T<:TorchNumber}
    return Tensor(a0[]; requires_grad=requires_grad)
end

function tensor_from_ptr(p::Ptr)
    n_dims = ccall((:tensor_method_ndimension, :libtorch_capi), Clonglong, (Ptr{Cvoid},), p)
    # sizes = zeros(Int64, n_dims)
    # ccall((:tensor_method_sizes, :libtorch_capi),
    #       Cvoid, (Ptr{Cvoid}, Ptr{Cvoid}),
    #       p, sizes)
    dtype = ccall((:tensor_method_dtype, :libtorch_capi), Cchar, (Ptr{Cvoid},), p)
    return Tensor{REVERSE_TYPE_MAP[dtype],n_dims}(p, nothing)
end
...
function Base.convert(::Type{Array}, t::Tensor{T,N}) where {T,N}
    ...
    ccall(
        (:tensor_method_data_copy, :libtorch_capi),
        Cvoid,
...
function Base.string(t::Tensor)
    str = ccall((:tensor_to_string, :libtorch_capi), Ptr{UInt8}, (Ptr{Cvoid},), t.pointer)
...
function Base.strides(t::Tensor)
    n_dims = ccall(
        (:tensor_method_ndimension, :libtorch_capi), Clonglong, (Ptr{Cvoid},), t.pointer
    )
    strides = zeros(Int64, n_dims)
    ccall(
        (:tensor_method_strides, :libtorch_capi),
        Cvoid,
        (Ptr{Cvoid}, Ptr{Cvoid}),
        t.pointer,
        strides,
...
function Base.size(t::Tensor{T,N}) where {T,N}
    ...
    sizes = zeros(Int64, n_dims)
    ccall(
        (:tensor_method_sizes, :libtorch_capi),
        Cvoid,
        (Ptr{Cvoid}, Ptr{Cvoid}),
        t.pointer,
        sizes,
...
function Base.setindex!(t::Tensor{T}, v::TorchNumber, i::Int64) where {T}
    return setindex!(t, Tensor{T}([v]), (eachindex(t)[i].I)...)
end
function Base.setindex!(t::Tensor{T,1}, v::TorchNumber, i::Int64) where {T}
    return setindex!(t, Tensor{T}([v]), i)
end
...
function Base.iterate(t::Tensor, state=(eachindex(t),))
    ...
    return t[y[1]], (state[1], Base.tail(y)...)
end

Base.cat(as::Tensor{T}...; dims) where {T} = Base._cat(dims, as...) |> Tensor
Base.hcat(as::Tensor{T}...) where {T} = Base.typed_hcat(T, as...) |> Tensor
Base.vcat(as::Tensor{T}...) where {T} = Base.typed_vcat(T, as...) |> Tensor
...
function item(t::Tensor{T,N}) where {T,N}
    @assert(N == 0 || prod(size(t)) == 1, "The Tensor must contain only one element.")
    data = T[zero(T)]
    ccall(
        (:tensor_method_item, :libtorch_capi),
        Cvoid,
        (Ptr{Cvoid}, Cchar, Ptr{Cvoid}),
        t.pointer,
        TYPE_MAP[T],
        data,
    )
...
function to(t::Tensor, d::Device, ::Type{T}) where {T<:TorchNumber}
    return THC.to4(t, convert(Int, d), eltype_id(T), 0, 0)
end

function on(t::Tensor)
    data = Int64[0, 0]
    ccall(
        (:tensor_method_device, :libtorch_capi),
        Cvoid,
        (Ptr{Cvoid}, Ptr{Cvoid}),
        t.pointer,
        data,
