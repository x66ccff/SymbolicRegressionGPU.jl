...
function has_grad(a::Tensor)
    ret = ccall((:tensor_method_has_grad, :libtorch_capi), Cint, (Ptr{Cvoid},), a.pointer)
    return ret != 0
end
...
function backward(
    a::Tensor,
    d::Union{Ptr{Nothing},Tensor}=C_NULL;
    ...
    ccall(
        (:tensor_method_backward, :libtorch_capi),
        Ptr{Cvoid},
...
function forward(f, args...)
    ...
    return y, (d) -> getindex.(Ref(back(d)), args)
end
